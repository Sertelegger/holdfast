...
11 MCP tools — `precheck_command`, `start_session` (with two-phase preflight), `send_input`, `request_secret_input`, `read_output`, `wait_for_pattern`, `resize`, `interrupt`, `terminate`, `list_sessions`, `status`. Full hybrid mode on Linux/macOS/WSL, stdio-only on Windows. `clasp attach`/`watch`/`list`/`logs`/`ui` CLI subcommands. Web UI with `xterm.js` (Unix-socket-only daemon; TCP bridge via `clasp ui` with bearer-token + Origin/Host validation). Gitleaks-derived secret redaction at every output boundary including audit logs. Argv-aware dangerous-command preflight with optional `strict_confirmation` (trusted-client redemption) mode. `request_secret_input` for out-of-band secret entry via attached clients (CLI attach `SecretInput` frame, web UI masked input). Process-group signal semantics (`setsid` Unix / job objects Windows). Multi-signal prompt detection with new combiner (`confidence = quiescent_score * pattern_score`). Bounded `read_output` responses with bulk output as MCP resources. Comprehensive testing across unit / integration / cross-platform CI / adversarial tiers.
