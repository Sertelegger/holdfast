...
# — nor, before the marker landed, either of them from a bound that
...
# a lone guard. What that row cannot reach is whether the schema survives
# serialisation to the wire at all, which is this script's standing job.
#
# The description is checked rather than only the type, because what
# makes this field worth anything is that it tells the agent what to do:
# a caller that reads a cause and is not told the boundary moves is in
# the same position it was in before the field existed.
jcheck "read_output declares held_back_cause and says what to do with it" \
