...
| `plugin` | The plugin and marketplace layer: `scripts/plugin-manifest-check.py` and its twelve breakage fixtures, shellcheck plus `dash -n`/`sh -n` over every shell file the plugin ships, and the bootstrap's safe-extraction rules against a generated corpus of 19 hostile tar archives and 11 hostile zips. **Three tar cells, and the third is on `macos-native`**: dash + GNU tar unprivileged, busybox ash + busybox tar as root in a digest-pinned Alpine container, and bsdtar — which is what macOS ships as `tar`. The cells are not repeats of each other; each check is deleted in turn and the corpus must go red, and two of the four checks are invisible outside the busybox-as-root cell. Then the download path itself, against a fabricated release served over loopback HTTP, because no real release carries binaries and `release.yml` attaches none on purpose |
