...
/// GH #203 put a floor under `limits.resource_read_max_bytes` — it must
/// cover the whole session ring, or `resources/read` stops being the
/// recourse from GH #195's wedge — so `= 8` is now refused at load and a
/// legal ceiling is at least 1 MiB, which a bash prompt's buffer cannot
/// reach past. The clamp itself is unchanged and is driven here from the
/// **caller's** `?max_bytes=`, which is the other half of the same
/// `min`; `mcp/resources.rs`'s
/// `a_caller_max_bytes_is_clamped_down_against_the_ceiling_and_never_up`
/// still drives the configured ceiling directly, and
/// `config::tests::a_resource_ceiling_under_the_live_ring_is_refused_and_
/// names_the_key` is what now pins the config side.
///
