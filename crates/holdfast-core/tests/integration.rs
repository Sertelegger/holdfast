...
/// until Task 8.
fn wait_for_buffer(session: &holdfast_core::session::Session, needle: &str) -> String {
...
async fn start_session_returns_a_session_id() {
    ...
    server
        ...
        .get(id)
        .unwrap()
        .signal(holdfast_core::pty::Signal::Kill)
        .unwrap();
}
...
async fn get_command_history_bounds_its_response_at_the_ring_default() {
    ...
    assert_eq!(
        indices.len(),
        holdfast_core::detect::DEFAULT_MAX_ENTRIES,
...
async fn an_env_value_that_matches_no_rule_is_still_absent() {
    ...
    let rules = holdfast_core::output::rules::builtin_shared();
    assert_eq!(
        holdfast_core::output::redact::redact_str(&rules, PASSPHRASE),
        PASSPHRASE,
