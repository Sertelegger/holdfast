...
impl Config {
    ...
    pub fn validate(&self) -> Result<(), ConfigError> {
        ...
        // against a constant rather than against the key beside it
        // (GH #203).**
        //
        // `resources/read` is the bulk counterpart to a cursor read:
        // same processor, same rules, same audit, over the whole ring
        // rather than a page of it (§5.5.5). A caller reaches for it
        // exactly when the incremental loop is not the right shape — a
        // backlog to swallow whole, a held-back tail to take in one
        // piece — and it can only be that while its ceiling covers the
        // ring it is supposed to be the bulk read of. Below that it is
        // a second paged read wearing a different name, and nothing
        // says so.
        //
        // It holds today by arithmetic that nothing asserted: the
        // default ceiling is 4 MiB and the live ring is 1 MiB. Lowering
        // this key is a legal, documented edit, and `nonzero` was its
        // only floor — so `= 1` loaded, with no error and no warning.
        //
        ...
        // `DEFAULT_BUFFER_BYTES` (GH #128). Relating a live key to a
        // dead one would let an operator who set `output_buffer_bytes =
        // 64 KiB` and `resource_read_max_bytes = 128 KiB` pass
        // validation while the real ring stayed at 1 MiB — a false
        // assurance, which is worse than no check and is the exact
        // class GH #128 exists to close.
        at_least(
            "limits.resource_read_max_bytes",
            l.resource_read_max_bytes,
    ...
    if value < floor {
        return Err(ConfigError::invalid(format!(
            "{key} = {value}, which is smaller than the {floor}-byte session \
             output ring; a resource read is the bulk counterpart to a cursor \
             read and can only be one while it reaches the whole ring, so \
             {key} must stay at or above {floor}"
        )));
    }
    Ok(())
}
...
";
    ...
    /// refused at load, because it silently turns the bulk read into a
    /// second paged one.
    ///
    ...
    /// stayed at 1 MiB. It must be rejected, and that is what separates
    /// this check from the one that reads as obviously right.
    #[test]
