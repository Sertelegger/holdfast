...
//!
//! Everything here runs against `CARGO_BIN_EXE_clasp`, so it exercises
...
const BIN: &str = env!("CARGO_BIN_EXE_clasp");
...
/// `crates/clasp-core/data/redaction_default.toml`. A fixture with a
...
fn the_shim_auto_spawns_a_daemon_and_exposes_the_daemons_tool_list() {
    ...
    let mut expected: Vec<String> = clasp_core::mcp::passthrough::passthrough_tools();
    expected.sort();
